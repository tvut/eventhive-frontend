<script>
	...
	let config = { 'Content-Type': 'Content-Type: application/json' };
	...
	const register = async () => {
		...
		axios
			...
			.catch((err) => {
                if(err.status == 400) {
					errorMessage = "Please try another username."
				} else {
					console.log(err)
	...
	<div class="flex flex-col items-center justify-center px-6 py-8 mx-auto md:h-screen lg:py-0">
		...
		<div class="w-full bg-white rounded-lg shadow md:mt-0 sm:max-w-md xl:p-0">
			<div class="p-6 space-y-4 md:space-y-6 sm:p-8">
				...
				<form on:submit|preventDefault={register} class="space-y-4 md:space-y-6" action="#">
					<div>
					...
					{#if errorMessage}
					<p class="text-sm text-red mt-0">{errorMessage}</p>
					{/if}
					<button type="submit" class="w-full text-white bg-red hover:bg-rose-800 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">Register</button>
